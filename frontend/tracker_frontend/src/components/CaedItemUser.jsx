...
//   })
  ...
  function handleDelete(){
    ...
    useEffect(()=>{
        axios.get(`http://127.0.0.1:8000/TracKer/project/${carddetails.list_mapped_to}/`,{ //{projectid}
            headers: {
              Authorization: 'Bearer ' + localStorage.getItem('access_token'),
          ...
          .then(function (response) {
            ...
            setProjectMaintainers(response.data.project_maintained_by.map((maintainer)=>{
                return{
                    label : maintainer.name,
                    value : maintainer.id
                }
